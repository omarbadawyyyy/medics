...
class PneumoniaAnalysisPage extends StatefulWidget {
  @override
  _PneumoniaAnalysisPageState createState() => _PneumoniaAnalysisPageState();
}

class _PneumoniaAnalysisPageState extends State<PneumoniaAnalysisPage> {
  ...
  Future<void> _loadModel() async {
    try {
      _interpreter = await Interpreter.fromAsset('assets/model_pneumonia.tflite');
  ...
  Future<void> _runInference() async {
    ...
    String result;
    if (outputShape[1] == 2) {
      result = output[0][0] > output[0][1] ? 'Negative (No Pneumonia)' : 'Positive (Pneumonia Detected)';
    } else {
  ...
  // دالة لعرض معلومات إضافية بناءً على النتيجة مع رسالة تحذير للحالة الإيجابية
  Widget _buildResultInfo(String result) {
    if (result.contains('Positive (Pneumonia Detected)')) {
      return Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Warning: Pneumonia detected! This is a serious condition.',
    ...
    } else if (result.contains('Negative (No Pneumonia)')) {
      return Container(
        padding: EdgeInsets.all(10),
        width: double.infinity,
        decoration: BoxDecoration(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Good News! No pneumonia detected. Regular check-ups are recommended.',
