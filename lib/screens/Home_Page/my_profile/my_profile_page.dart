...
import 'package:medics/screens/Home_Page/my_profile/screens_myProfil/my_account_screens/my_account_page.dart';
...
class _ProfilePageState extends State<MyProfilePage> {
  ...
  _loadUserData() async {
    ...
    setState(() {
      name = prefs.getString('name_${widget.email}') ?? '';
      phoneNumber = prefs.getString('phone_${widget.email}') ?? '';
    });
  }
  ...
  Future<void> _pickImage() async {
    ...
    if (image != null) {
      ...
      SharedPreferences prefs = await SharedPreferences.getInstance();
      await prefs.setString('profileImagePath_${widget.email}', path);
  ...
  PageRouteBuilder _createSlideRoute(Widget page) {
    return PageRouteBuilder(
      ...
      transitionsBuilder: (context, animation, secondaryAnimation, child) {
        const begin = Offset(1.0, 0.0); // البداية من اليمين
        const end = Offset.zero; // النهاية في الوسط
            ...
            children: [
              ...
              Column(
                children: [
                  ...
                  _buildListTile('My Account', Icons.account_box, onTap: () {
                    Navigator.push(
                      context,
                      _createSlideRoute(MyAccountPage(email: widget.email)),
                    );
                  }),
                  Divider(color: Colors.grey[300], thickness: 1, indent: 16, endIndent: 16),
